...
namespace DDMSSense.DDMS.Summary {
	...
	/// 
	/// <para>
	/// DDMSence is stricter than the specification in the following ways:</para><ul>
	...
	/// </ul></p>
	/// 
	/// <para>The above design decision dictates that VerticalDistance (the type behind minVerticalExtent and maxVerticalExtent) 
	/// does not need to be implemented as a Java class.</para>
	/// 
	...
	public sealed class VerticalExtent : AbstractBaseComponent {
		...
		private static List<string> VERTICAL_DATUM_TYPES = new List<string>();
		...
		private static List<string> LENGTH_MEASURE_TYPES = new List<string>();
		...
		public VerticalExtent(double minVerticalExtent, double maxVerticalExtent, string unitOfMeasure, string datum) {
			try {
				...
				element.Add(Util.BuildDDMSElement(MinVerticalExtentName, Convert.ToString(minVerticalExtent)));
				element.Add(Util.BuildDDMSElement(MaxVerticalExtentName, Convert.ToString(maxVerticalExtent)));
		...
		protected internal override void Validate() {
			...
			Util.RequireDDMSValue(MaxVerticalExtentName, MaxVerticalExtent);
			Util.RequireDDMSValue(UOM_NAME, UnitOfMeasure);
			Util.RequireDDMSValue(DATUM_NAME, Datum);
		...
		private void ValidateInheritedAttributes(Element extentElement) {
			...
			if (!String.IsNullOrEmpty(unitOfMeasure) && !unitOfMeasure.Equals(UnitOfMeasure)) {
				throw new InvalidDDMSException("The unitOfMeasure on the " + extentElement.Name.LocalName + " element must match the unitOfMeasure on the enclosing verticalExtent element.");
			}
			if (!String.IsNullOrEmpty(datum) && !datum.Equals(Datum)) {
				throw new InvalidDDMSException("The datum on the " + extentElement.Name.LocalName + " element must match the datum on the enclosing verticalExtent element.");
			}
		}
		...
		public override bool Equals(object obj) {
			...
			return (UnitOfMeasure.Equals(test.UnitOfMeasure) && Datum.Equals(test.Datum) && MinVerticalExtent.Equals(test.MinVerticalExtent) && MaxVerticalExtent.Equals(test.MaxVerticalExtent));
		}
		...
		private string MinVerticalExtentName {
			get {
				return (DDMSVersion.IsAtLeast("4.0.1") ? "minVerticalExtent" : "MinVerticalExtent");
			}
		}
		...
		private string MaxVerticalExtentName {
			get {
				return (DDMSVersion.IsAtLeast("4.0.1") ? "maxVerticalExtent" : "MaxVerticalExtent");
			}
		}
		...
		public class Builder : IBuilder {
			internal const long SerialVersionUID = 5188383406608210723L;
			...
			public virtual VerticalExtent Commit() {
				...
				return (new VerticalExtent((double)MinVerticalExtent, (double)MaxVerticalExtent, UnitOfMeasure, Datum));
			}

			/// <seealso cref= IBuilder#isEmpty() </seealso>
			public virtual bool Empty {
				get {
					return (MinVerticalExtent == null && MaxVerticalExtent == null && String.IsNullOrEmpty(UnitOfMeasure) && String.IsNullOrEmpty(Datum));
				}
			}
