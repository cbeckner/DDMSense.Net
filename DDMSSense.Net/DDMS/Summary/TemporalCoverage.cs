...
namespace DDMSSense.DDMS.Summary {
	...
	/// </para>
	/// 
	/// <para>Starting in DDMS 4.1, the start and end dates may optionally be replaced by an approximableStart
	/// or approximableEnd date.</para>
	/// 
	/// <para>To avoid confusion between the name of the temporalCoverage element and the name of the specified time period,
	...
	public sealed class TemporalCoverage : AbstractBaseComponent {
		...
		private static List<string> EXTENDED_DATE_TYPES = new List<string>();
		...
		private TemporalCoverage(string timePeriodName, string startString, ApproximableDate approximableStart, string endString, ApproximableDate approximableEnd, SecurityAttributes securityAttributes) {
			try {
				...
				Element periodElement = DDMSVersion.GetCurrentVersion().IsAtLeast("4.0.1") ? element : Util.BuildDDMSElement(TIME_PERIOD_NAME, null);
				...
				if (approximableStart != null) {
					...
					periodElement.Add(Util.BuildDDMSElement(START_NAME, startString));
				}
				...
				if (approximableEnd != null) {
					...
					periodElement.Add(Util.BuildDDMSElement(END_NAME, endString));
				}
		...
		protected internal override void Validate() {
			...
			if (ApproximableStart == null) {
				Util.RequireDDMSValue("start", StartString);
				if (!EXTENDED_DATE_TYPES.Contains(StartString)) {
					Util.RequireDDMSDateFormat(StartString, Namespace);
				}
			}
			if (ApproximableEnd == null) {
				Util.RequireDDMSValue("end", EndString);
				if (!EXTENDED_DATE_TYPES.Contains(EndString)) {
					Util.RequireDDMSDateFormat(EndString, Namespace);
				}
			}
		...
		protected internal override void ValidateWarnings() {
			Element periodElement = TimePeriodElement;
			Element timePeriodName = periodElement.Element(XName.Get(TIME_PERIOD_NAME_NAME, periodElement.Name.NamespaceName));
		...
		protected internal override string LocatorSuffix {
			get {
                return (DDMSVersion.IsAtLeast("4.0.1") ? "" : ValidationMessage.ELEMENT_PREFIX + Element.Prefix + ":" + TIME_PERIOD_NAME);
			}
		}
		...
		public override bool Equals(object obj) {
			...
			return (TimePeriodName.Equals(test.TimePeriodName) && StartString.Equals(test.StartString) && EndString.Equals(test.EndString));
		}
		...
		private Element TimePeriodElement {
			get {
				return (DDMSVersion.IsAtLeast("4.0.1") ? Element : GetChild(TIME_PERIOD_NAME));
			}
		}
		...
		public class Builder : IBuilder {
            ...
            public virtual IDDMSComponent Commit()
            {
				...
				return (new TemporalCoverage(TimePeriodName, StartString, (ApproximableDate)ApproximableStart.Commit(), EndString, (ApproximableDate)ApproximableEnd.Commit(), (SecurityAttributes)SecurityAttributes.Commit()));
			}

			/// <seealso cref= IBuilder#isEmpty() </seealso>
			public virtual bool Empty {
				get {
					return (String.IsNullOrEmpty(TimePeriodName) && String.IsNullOrEmpty(StartString) && String.IsNullOrEmpty(EndString) && ApproximableStart.Empty && ApproximableEnd.Empty && SecurityAttributes.Empty);
				}
			}
