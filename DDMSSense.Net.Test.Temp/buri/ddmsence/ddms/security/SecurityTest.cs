...
namespace buri.ddmsence.ddms.security {
	...
	using Access = buri.ddmsence.ddms.security.ntk.Access;
		...
		public virtual void TestWarnings() {
			foreach (string sVersion in SupportedVersions) {
				...
				if (version.isAtLeast("4.0.1")) {
					Element element = Util.buildDDMSElement(Security.getName(version), null);
					Util.addAttribute(element, PropertyReader.getPrefix("ism"), "excludeFromRollup", version.IsmNamespace, "true");
					Element accessElement = Util.buildElement(PropertyReader.getPrefix("ntk"), Access.getName(version), version.NtkNamespace, null);
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void testWrongVersionExcludeFromRollup() throws buri.ddmsence.ddms.InvalidDDMSException
